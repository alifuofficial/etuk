...
const navItems = [
  ...
  { href: '/accountant/proformas', label: 'Proformas', icon: FileText },
  { href: '/accountant/verify', label: 'Verify Payments', icon: CheckCircle },
];
...
export default function AccountantLayout({
  ...
  useEffect(() => {
    ...
    // Only ACCOUNTANT can access this area
    if (status === 'authenticated' && session?.user?.role !== 'ACCOUNTANT') {
      router.push('/admin');
    }
  ...
  if (!session || session.user.role !== 'ACCOUNTANT') return null;
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div className="h-16 flex items-center px-6 border-b border-gray-100">
            <Link href="/accountant" className="flex items-center gap-3">
              <img 
              ...
              <span className="font-bold text-xl tracking-tight text-gray-900">
                ETUK <span className="text-emerald-600 text-xs font-medium tracking-normal">Accountant</span>
              </span>
            </Link>
          </div>
        ...
        <header className="sticky top-0 z-30 h-16 bg-white border-b border-gray-200 px-6 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-sm font-semibold text-gray-900 capitalize hidden sm:block">
              {pathname === '/accountant' ? 'Dashboard' : pathname.split('/').pop()?.replace('-', ' ')}
            </h2>
          </div>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="flex items-center gap-2 p-1 pl-2 rounded-full border border-gray-200 hover:bg-gray-50 transition-colors">
                  ...
                  <Avatar className="w-8 h-8">
                    <AvatarFallback className="bg-emerald-600 text-white text-xs font-bold uppercase">
              ...
              <DropdownMenuContent align="end" className="w-56 mt-2 rounded-xl border-gray-200 p-1">
                ...
                <div className="px-3 py-2 mb-1">
                  ...
                  <span className="inline-flex items-center px-2 py-0.5 mt-1 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-700 border border-emerald-200">
                    ACCOUNTANT
                  </span>
                </div>
