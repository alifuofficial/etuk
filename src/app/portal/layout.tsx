    ...
    <div className="flex flex-col h-full">
      ...
      <div className="h-16 flex items-center px-6 border-b border-gray-100">
        <Link href="/" className="flex items-center gap-3">
          ...
          />
          <span className="font-bold text-lg tracking-tight text-gray-900">Soreti <span className="text-gray-400 text-xs font-medium">Agent</span></span>
        </Link>
      </div>
...
function Header({ session, sidebarOpen, setSidebarOpen }: { session: any; sidebarOpen: boolean; setSidebarOpen: (open: boolean) => void }) {
  const pathname = usePathname();
  const router = useRouter();
      ...
      <div className="flex items-center gap-3">
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button className="flex items-center gap-2 p-1 pl-2 rounded-full border border-gray-200 hover:bg-gray-50 transition-colors">
              ...
              <Avatar className="w-8 h-8">
                <AvatarFallback className="bg-green-600 text-white text-xs font-bold uppercase">
          ...
          <DropdownMenuContent align="end" className="w-56 mt-2 rounded-xl border-gray-200 p-1">
            ...
            <DropdownMenuItem onClick={() => router.push('/portal/settings')} className="p-2 rounded-lg text-gray-700 focus:bg-gray-50 cursor-pointer">
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans">
      ...
      <div className="lg:pl-64 flex flex-col min-h-screen">
        <Header session={session} sidebarOpen={sidebarOpen} setSidebarOpen={setSidebarOpen} />
